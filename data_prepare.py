...
def prepare_data(
    ...
    for split_index in range(len(splits)):
        ...
        text_lst = get_all_files(
            os.path.join(data_folder, split), match_and=[".txt"]
        )
...
def create_csv(
    ...
    for wav_file in tzip(wav_lst):
        ...
        snt = wav_file.split("/")[-2:]
        snt_id = "-".join(snt).replace(".wav", "")
        #spk_id = "-".join(snt_id.split("-")[0:2])
        spk_id = snt_id.split('-')[-1]
        wrds = text_dict[spk_id]
...
def text_to_dict(text_lst):
    ...
    for file in text_lst:
        with open(file, "r", encoding='UTF-8') as f:
            ...
            for line in f:
                line1 = re.sub(chars_to_remove_regex, '', line).lower()
                line_lst = line1.strip().split(" ")
